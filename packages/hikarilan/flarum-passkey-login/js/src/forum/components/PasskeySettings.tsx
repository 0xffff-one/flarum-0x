...
export default class PasskeySettings extends Component<ComponentAttrs, IState> {
  ...
  async oncreate(vnode: Mithril.VnodeDOM<ComponentAttrs, this>): Promise<void> {
  ...
  view(vnode: Mithril.Vnode<ComponentAttrs, this>): Mithril.Children {
    ...
    return (
      <FieldSet label={app.translator.trans('hikarilan-passkey-login.forum.user.settings.passkeys.title')}>
        <span className="helpText">{app.translator.trans('hikarilan-passkey-login.forum.user.settings.passkeys.label')}</span>
        {this.state.loading ? (
